...
{
    private $_columns = array('id', 'fullname', 'email', 'title', 'phone', 'created', 'created_by', 'modified', 'modified_by', 'status', 'ordering');
    private $_userInfo;
    ...
    {
        $query[]    = "SELECT `id`, `fullname`, `email`, `title`, `content` ,`phone`, `created`, `created_by`, `modified`, `modified_by`, `status`, `ordering`";
        $query[]    = "FROM `$this->table`";
        $query[]    = "WHERE `id` > 0";
    ...
    public function changeStatus($arrParam, $option = null)
    {
        if ($option['task'] == 'change-ajax-status') {
            ...
            $result = array(
                'id'        => $id,
                'status'    => $status,
                'link'        => URL::createLink('admin', 'contact', 'ajaxStatus', array('id' => $id, 'status' => $status))
            );
            return $result;
        }
    ...
    {
        if ($option == null) {
            $query[]    = "SELECT 'id', 'fullname', 'email', 'title', 'phone', 'status', 'ordering'";
            $query[]    = "FROM `$this->table`";
