...
class SliderModel extends Model
{
    ...
    private $_columns = array('id', 'title', 'description', 'image', 'created', 'created_by', 'modified', 'modified_by', 'status', 'ordering', 'link');
    private $_userInfo;
    ...
    {
        if ($option == null) {
            $query     = "SELECT `id`, `name` FROM `" . TBL_SLIDER . "`";
    ...
    {
        $query[]    = "SELECT `id`, `title`, `description`, `image`, `status`, `ordering`, `link`,`created`, `created_by`, `modified`, `modified_by`";
        $query[]    = "FROM `$this->table`";
        $query[]    = "WHERE `id` > 0";
    ...
    public function deleteItem($arrParam, $option = null)
    {
        if ($option == null) {
            if (!empty($arrParam['cid'])) {
                ...
                // Remove image
                $query       = "SELECT `id`, `image` AS `name` FROM `$this->table` WHERE `id` IN ($ids)";
    ...
    {
        if ($option == null) {
            $query[]    = "SELECT `id`, `title`, `description`, `image`, `created`, `created_by`, `modified`, `modified_by`, `status`, `ordering`,`link`";
            $query[]    = "FROM `$this->table`";
    ...
    public function saveItem($arrParam, $option = null)
    {
        ...
        $uploadObj = new Upload();
        if ($option['task'] == 'add') {
            $arrParam['form']['image']    = $uploadObj->uploadFile($arrParam['form']['picture'], 'slider');
            ...
            $arrParam['form']['link']        = mysqli_real_escape_string($this->connect, $arrParam['form']['link']);
        ...
        if ($option['task'] == 'edit') {
            if ($arrParam['form']['picture']['name'] == null) {
                ...
                $arrParam['form']['picture']    = $uploadObj->uploadFile($arrParam['form']['picture'], 'slider', 98, 150);
            }
