...
class BlogModel extends Model
{
    ...
    {
        if ($option == null) {
            $query     = "SELECT `id`, `name` FROM `" . TBL_BLOG . "`";
    ...
    {
        $query[]    = "SELECT `b`.`id`,  `b`.`name`, `b`.`picture`, `b`.`content`, `b`.`status`, `b`.`ordering`, `b`.`created`, `b`.`created_by`, `b`.`modified`, `b`.`modified_by`";
        ...
        if ($option == null) {
            if (!empty($arrParam['cid'])) {
                ...
                foreach ($arrImage as $value) {
                    $uploadObj->removeFile('blog', $value);
                    $uploadObj->removeFile('blog', '98x150-' . $value);
                }
    ...
    {
        if ($option == null) {
            $query[]    = "SELECT `id`, `content`, `picture`,`name`, `status`, `ordering`";
            $query[]    = "FROM `$this->table`";
    ...
    public function saveItem($arrParam, $option = null)
    {
        ...
        if ($option['task'] == 'add') {
            ...
            $arrParam['form']['content']        = mysqli_real_escape_string($this->connect, $arrParam['form']['content']);
        ...
        if ($option['task'] == 'edit') {
            if ($arrParam['form']['picture']['name'] == null) {
                ...
                $uploadObj->removeFile('blog', '98x150-' .  $arrParam['form']['picture_hidden']);
