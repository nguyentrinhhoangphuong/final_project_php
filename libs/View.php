...
class View
{
	...
	// SET Vendor CSS
	public function appendVendorCSS($arrayVendorCSS)
	{
		if (!empty($arrayVendorCSS)) {
			foreach ($arrayVendorCSS as $vendorCss) {
				$file = APPLICATION_URL . $this->_moduleName . DS . 'views' . DS . $vendorCss;
				$this->_cssFiles .= '<link rel="stylesheet" type="text/css" href="' . $file . '"/>';
			}
		}
	}
	...
	// SET Vendor JS
	public function appendVendorJs($arrayVendorJs)
	{
		if (!empty($arrayVendorJs)) {
			foreach ($arrayVendorJs as $vendor) {
				$file = APPLICATION_URL . $this->_moduleName . DS . 'views' . DS . $vendor;
				$this->_jsFiles .= '<script type="text/javascript" src="' . $file . '"></script>';
			}
		}
	}
